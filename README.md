...
This project provides a Docker container for [dupeGuru](https://dupeguru.voltaicideas.net).
...
Where:
  ...
  - `/docker/appdata/dupeguru`: Stores the application's configuration, state, logs, and any files requiring persistency.
  - `/home/user`: Contains files from the host that need to be accessible to the application.

Access the dupeGuru GUI by browsing to `http://your-host-ip:5800`.
...
[Releases]: https://github.com/jlesage/docker-dupeguru/releases
[Docker Hub]: https://hub.docker.com/r/jlesage/dupeguru/tags
...
container, it would proxy all HTTP requests for `dupeguru.domain.tld` to
...
server {
	...
	location /dupeguru/ {
		...
		# Needed when audio support is enabled.
		location /dupeguru/websockify-audio {
			proxy_pass http://docker-dupeguru/websockify-audio;
			proxy_http_version 1.1;
			proxy_set_header Upgrade $http_upgrade;
			proxy_set_header Connection $connection_upgrade;
			proxy_read_timeout 86400;
...
| **Application version** label | Displays the version of dupeGuru integrated into Docker image. |
...
Any desktop notification generated by dupeGuru is forwarded to the
